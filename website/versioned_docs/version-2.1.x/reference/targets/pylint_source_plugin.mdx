...
<Field type_repr={`field.type_hint`}>
  Addresses to other targets that this plugin depends on. Due to restrictions
  with Pylint plugins, these targets must either be third-party Python
  dependencies (https://www.pantsbuild.org/docs/python-third-party-dependencies)
  or be located within this target&#x27;s same directory or a subdirectory.
</Field>
