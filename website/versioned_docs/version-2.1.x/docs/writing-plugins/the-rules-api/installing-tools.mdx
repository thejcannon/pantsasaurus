...
:::note Use `PythonToolBase` when you need a Subsystem
    ...
    Often, you will want to create a [`Subsystem`](doc:rules-api-subsystems) for your Python tool to allow users to set options like `--black-config`. You can subclass `PythonToolBase`—which subclasses `Subsystem`—to do this:
