...
[block:callout]
{
  ...
  "body": "This guide walks through each step of adding Shellcheck. See [here](https://github.com/pantsbuild/example-plugin/blob/main/pants-plugins/examples/bash/lint/shellcheck) for the final result."
}
[/block]
...
[block:api-header]
{
  "title": "2. Set up a `FieldSet` and `LintRequest`"
}
[/block]
...
Finally, register your new `LintRequest` with a [`UnionRule`](doc:rules-api-unions) so that Pants knows your linter exists:
...
[block:api-header]
{
  "title": "3. Create a rule for your linter logic"
}
[/block]
...
[block:code]
{
  "codes": [
    {
      "code": "from bash import shellcheck\n\n\ndef rules():\n    return [*shellcheck.rules()]",
      "language": "python",
