...
[block:parameters]
{
  "data": {
    ...
    "3-1": "string",
    "3-2": "Path to a file containing an authorization token to pass in remote execution requests.  Necessary only if the server requires authentication. The token will be placed in the `authorization` header in gRPC requests as `Bearer TOKEN`.",
    "4-0": "`--remote_execution_extra_platform_properties`",
    "4-1": "list of strings",
    "4-2": "Key/value pairs for the platform properties to set on remote execution requests. Each list element should be in the form `KEY=VALUE`.",
    "5-0": "`--remote_ca_certs_path`",
    "5-1": "string",
    "5-2": "Path to a file containing the TLS root certificate store. The path varies by operating system.",
    "6-0": "`--process_execution_remote_parallelism`",
    "6-1": "integer",
    "6-2": "Maximum number of concurrent remote execution requests to submit to the REAPI server at any one time.",
    "h-0": "Option",
    "h-1": "Type",
    "h-2": "Description"
  },
