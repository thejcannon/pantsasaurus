...
```python title="title="helloworld/greet/BUILD""
python_library(
    name = 'greet',
    sources = ['*.py', '!*_test.py'],
    compatibility = ">=3.6",
...
```
See [Adopting Pants in an Existing Repo](doc:existing-repositories) for tips on setting up Pants, including where to add BUILD files.
