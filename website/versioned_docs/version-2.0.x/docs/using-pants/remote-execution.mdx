...
[block:callout]
{
  ...
  "body": "Remote execution support in Pants comes with several limitations. For example, Pants requires that the server's operating system match the client's operating system. In practice, this means that Pants must be running on Linux because all three major server projects really only operate well on Linux."
}
[/block]
...
[block:parameters]
{
  "data": {
    ...
    "h-2": "Description",
    "0-1": "boolean",
    "0-2": "This option turns remote execution on and off. It must be set to `true` for Pants to use remote execution.",
    "1-0": "`--remote-execution-server`",
    "1-1": "string",
    "1-2": "HOST:PORT of a REAPI-compliant execution service",
    "2-0": "`--remote-store-server`",
    "2-1": "string",
    "2-2": "HOST:PORT of a REAPI-compliant content-addressable storage service (\"CAS\")",
    "3-0": "`--remote_instance_name`",
    "3-1": "string",
    "3-2": "The name of the REAPI \"instance\" to pass to the REAPI server. An REAPI \"instance\" is a distinct deployment of the CAS and execution service given a unique name. This can be the empty string if the server supports such usage.",
    "4-0": "`--remote_oauth_bearer_token_path`",
    "4-1": "string",
    "4-2": "Path to a file containing an authorization token to pass in remote execution requests.  Necessary only if the server requires authentication. The token will be placed in the `authorization` header in gRPC requests as `Bearer TOKEN`.",
    "5-0": "`--remote_execution_extra_platform_properties`",
    "5-1": "list of strings",
    "5-2": "Key/value pairs for the platform properties to set on remote execution requests. Each list element should be in the form `KEY=VALUE`.",
    "6-0": "`--remote_ca_certs_path`",
    "6-1": "string",
    "6-2": "Path to a file containing the TLS root certificate store. The path varies by operating system.",
    "7-0": "`--process_execution_remote_parallelism`",
    "7-1": "integer",
    "7-2": "Maximum number of concurrent remote execution requests to submit to the REAPI server at any one time."
  },
  "cols": 3,
  "rows": 8
}
