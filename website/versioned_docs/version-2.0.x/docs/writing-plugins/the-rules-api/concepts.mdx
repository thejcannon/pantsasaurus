...
```
We know that rule graph errors can be intimidating and confusing to understand. We are planning to improve them. In the meantime, please do not hesitate to ask for help in the #engine channel on [Slack](doc:community).
...
```python title="title="pants-plugins/plugin1/register.py""
def rules():
    return [rule1, rule2]
```
...
<Tabs>
<TabItem value="pants-plugins/fortran/register.py" label="pants-plugins/fortran/register.py">
...
<TabItem value="pants-plugins/fortran/fmt.py" label="pants-plugins/fortran/fmt.py">
...
<TabItem value="pants-plugins/fortran/test.py" label="pants-plugins/fortran/test.py">
