...
```
While we'll try our best to limit changes, the Plugin API does not yet follow the [Deprecation Policy](doc:deprecation-policy). Components of the API may change between minor versions—e.g. 2.1.0 to 2.2.0—without a deprecation.
...
```python title="title="pants-plugins/BUILD""
...
```python title="title="pants-plugins/bash/BUILD""
python_library(
