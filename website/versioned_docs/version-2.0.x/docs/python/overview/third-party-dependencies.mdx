...
<TabItem value="project/app.py" label="project/app.py">
...
```python title="title="project/BUILD""
python_library(
    dependencies=[
...
If you define a lockfile, Pants will also optimize to avoid resolving requirements more than one time for your project. This greatly speeds up the performance of goals like `test`, `run`, and `repl`. (See [python-setup](doc:reference-python-setup) for more information on the `resolve_all_constraints` option.)
...
<TabItem value="constraints.txt" label="constraints.txt">
...
```
For now, Pants only supports one single global lockfile. But, we are considering adding support for multiple lockfiles. Please message us on [Slack](doc:community) if you would like this feature - we would appreciate feedback on your use case.
...
```shell title="title="build-support/generate_constraints.sh""
#!/usr/bin/env bash
