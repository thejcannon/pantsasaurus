...
<CaptionedImg src={require("/img/pants-concurrency.gif").default}>Pants running multiple linters in parallel.</CaptionedImg>
...
<CaptionedImg src={require("/img/pants-caching.gif").default}>We run both tests, then add a syntax error to one test and rerun; the unmodified test uses the cache and is isolated from the syntax error.</CaptionedImg>
### Remote Execution
