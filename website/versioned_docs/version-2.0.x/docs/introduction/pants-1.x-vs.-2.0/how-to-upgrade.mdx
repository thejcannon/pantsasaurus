...
```
See [Pants 1.x vs. 2.0](doc:pants-1-vs-2).
...
```toml title="title="pants.v1.toml""
[GLOBAL]
...
```
Please message us on [Slack](doc:community) if you are having issues upgrading or find something missing. We would love to help.
