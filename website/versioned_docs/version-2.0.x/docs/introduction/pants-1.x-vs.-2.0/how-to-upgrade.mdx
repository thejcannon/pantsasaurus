...
Previously, the `provides` field was used on `python_library` targets for the [`setup-py` goal](doc:reference-setup-py). Now, use a dedicated `python_distribution` target.
