...
<Field type_repr={`field.type_hint`}>
  Name of the release used in the deployment. If not set, the target name will
  be used instead.
</Field>
...
<Field type_repr={`field.type_hint`} default_repr={`('*.yaml', '*.yml')`}>
...
<Field type_repr={`field.type_hint`} default_repr={`False`}>
  If true, then does not deploy the Custom Resource Definitions that are defined
  in the chart.
</Field>
...
<Field type_repr={`field.type_hint`} default_repr={`False`}>
  If true, none of the lifecycle hooks of the given chart will be included in
  the deployment.
</Field>
