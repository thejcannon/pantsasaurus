...
<Field type_repr={`field.type_hint`}>The name of the binary to find.</Field>

## `extra_search_paths`

<Field type_repr={`field.type_hint`} default_repr={`()`}>
  Extra search paths to look for the binary. These take priority over
  Pants&#x27; default search paths.
</Field>
...
<Field type_repr={`field.type_hint`} default_repr={`()`}>
  Specifies arguments that will be used to run the binary during the search
  process.
</Field>
...
<Field type_repr={`field.type_hint`}>
  ...
  when the binary is run, so that the search process may complete successfully.
  The name of the target must be the name of the runnable dependency that is
  called by this binary.
</Field>
