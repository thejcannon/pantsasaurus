...
<Option
  ...
  default_repr={`True`}
>
  If true, then convert coverage reports to HTML format and write a
  `coverage.html` file next to the raw coverage data.
</Option>
...
<Option
  ...
  default_repr={`False`}
>
  If true, don&#x27;t use Go test binary when running `scie-pants-linux-x86_64
  test`.
</Option>
...
>
  ...
  The profile will be written to the file `block.out` in the test extra output
  directory. The test binary will also be written to the test extra output
  directory.
</Option>
...
<Option
  ...
  default_repr={`False`}
>
  Capture a CPU profile from the execution of the test runner. The profile will
  be written to the file `cpu.out` in the test extra output directory. The test
  binary will also be written to the test extra output directory.
</Option>
...
<Option
  ...
  default_repr={`False`}
>
  Capture an allocation profile from the execution of the test runner after
  tests have passed. The profile will be written to the file `mem.out` in the
  test extra output directory. The test binary will also be written to the test
...
<Option
  ...
  default_repr={`False`}
>
  Capture a mutex contention profile from the execution of the test runner when
  all tests are complete. The profile will be written to the file `mutex.out` in
  the test extra output directory. The test binary will also be written to the
...
<Option
  ...
  default_repr={`False`}
>
  Capture an execution trace from the execution of the test runner. The trace
  will be written to the file `trace.out` in the test extra output directory.
</Option>
