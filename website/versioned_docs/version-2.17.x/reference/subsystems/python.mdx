...
>
  ...
  See the relevant field for more details.
</Option>
...
<Option
  ...
  default_repr={`True`}
>
  If true, add `python_sources`, `python_tests`, and `python_test_utils` targets
  with the `tailor` goal.
</Option>
...
<Option
  ...
  default_repr={`False`}
>
  If true, add `pex_binary` targets for Python files named `__main__.py` or with
  a `__main__` clause with the `tailor` goal.
</Option>
...
<Option
  ...
  default_repr={`True`}
>
  If true, add `resource` targets for marker files named `py.typed` with the
  `tailor` goal.
</Option>
...
<Option
  ...
  default_repr={`False`}
>
  If set, and if running on MacOS Big Sur, use macosx_10_16 as the platform when
  building wheels. Otherwise, the default of macosx_11_0 will be used. This may
  be required for pip to be able to install the resulting distribution on Big
  Sur.
</Option>
