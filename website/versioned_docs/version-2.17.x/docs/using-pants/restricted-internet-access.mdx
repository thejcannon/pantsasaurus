...
To do so, set the [`repos`](reference:reference-coursier#section-repos) option on the `[coursier]` scope. E.g.,
...
`pants help-advanced protoc` (or its [online equivalent](reference:reference-protoc#advanced-options)) shows that the default URL template is `https://github.com/protocolbuffers/protobuf/releases/download/v{version}/protoc-{version}-{platform}.zip`.
...
:::note Asking for help
    ...
    Please reach out on [Slack](../getting-help/the-pants-community/index.mdx) or open a [GitHub issue](https://github.com/pantsbuild/pants/issues) for any help.
