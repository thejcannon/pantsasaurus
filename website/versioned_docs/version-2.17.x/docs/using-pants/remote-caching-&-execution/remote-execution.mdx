...
For global options, run `pants help-advanced global` or refer to [Global options](reference:reference-global). Most remote execution and caching options begin with the prefix `--remote`.

For environment-specific options, see `pants help-advanced remote_environment` or the [`remote_environment` target](reference:reference-remote_environment).
