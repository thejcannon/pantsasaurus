...
Pants, however, takes care of assembling the context for you. It does so using the dependencies of the [`docker_image`](../../reference/targets/docker-image.mdx) target, which can include:

- Loose files specified using  [`file` / `files` targets](../using-pants/assets-and-archives.mdx#files).
- Artifacts packaged from a variety of targets, such as [`pex_binary`](../../reference/targets/pex-binary.mdx) , [`python_distribution`](../../reference/targets/python-distribution.mdx), [`archive`](../../reference/targets/archive.mdx), and any other target that can be built via the [package](../../reference/goals/package.mdx) goal, including other docker images.
