...
the [`[jvm].jdk` option](reference:reference-jvm#section-jdk) to set a repository-wide default.

To override the default on a particular target, you can use the [`jdk=` field](reference:reference-kotlin_source#codejdkcode).
...
below) using the [`[kotlin].version_for_resolve` option](reference:reference-kotlin#section-version_for_resolve). The
...
the [`resolve` field](reference:reference-kotlin_junit_test#coderesolvecode) of any targets which should be used with the
...
[`jvm_artifact` targets](reference:reference-jvm_artifact):
...
[`[jvm].resolves` option](reference:reference-jvm#section-resolves).
...
:::note Thirdparty symbols and the `packages` argument
    ...
    [`jvm_artifact` docs](reference:reference-jvm_artifact#codepackagescode) for more information.
