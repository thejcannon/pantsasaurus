...
```text title="tab={"label":"requirements.txt"}"
pytest-django==3.10.0
```

```python title="tab={"label":"helloworld/util/BUILD"}"
python_tests(
   name="tests",
...
```python title="tab={"label":"test_debug_example.py"}"
def test_debug():
    import pdb; pdb.set_trace()
    assert 1 + 1 == 2
```

```text title="tab={"label":"Shell"}"
...
```python title="tab={"label":"helloworld/BUILD"}"
python_library(
    name="testutils",
...
```python title="tab={"label":"helloworld/testutils.py"}"
...
...
```python title="tab={"label":"helloworld/app_test.py"}"
...
```python title="tab={"label":"helloworld/say_hello.py"}"
print("Hello, test!")
```

```python title="tab={"label":"helloworld/test_binary.py"}"
import subprocess
...
```toml title="title="pants.ci.toml""
[test]
use_coverage = true
```
...
```toml title="tab={"label":"pants.toml"}"
[coverage-py]
config = ".coveragerc"
```

```text title="tab={"label":".coveragerc"}"
[run]
relative_files = True
branch = True
```
