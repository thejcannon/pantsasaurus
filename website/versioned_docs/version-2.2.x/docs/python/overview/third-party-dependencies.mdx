...
If you define a lockfile, Pants will also optimize to avoid resolving requirements more than one time for your project. This greatly speeds up the performance of goals like `test`, `run`, and `repl`. (See [python-setup](reference:reference-python-setup) for more information on the `resolve_all_constraints` option.)
