...
[block:callout]
{
  ...
  "body": "This guide walks through each step of adding shfmt. See [here](https://github.com/pantsbuild/example-plugin/blob/main/pants-plugins/examples/bash/lint/shfmt) for the final result."
}
[/block]
...
[block:api-header]
{
  "title": "2. Set up `LanguageFmtTargets` and `FmtRequest`"
}
[/block]
...
{
  "type": "info",
  "title": "New Python formatters can skip this step",
  "body": "Pants already defines `PythonFmtTargets` and `PythonFmtRequest`, which are both defined in `pants.backend.python.lint.python_fmt`."
}
[/block]
...
[block:api-header]
{
  "title": "3. Set up a `FieldSet` and `FmtRequest`/`LintRequest`"
}
[/block]
...
Finally, register your new `LintRequest`/`FmtRequest` with two [`UnionRule`s](doc:rules-api-unions) so that Pants knows your formatter exists:
...
[block:code]
{
  "codes": [
    {
      "code": "from bash import bash_formatters, shfmt\n\n\ndef rules():\n    return [*bash_formatters.rules(), *shfmt.rules()]",
      "language": "python",
