...
Pants uses [`shell_source`](reference:reference-shell_source) and [`shunit2_test`](reference:reference-shunit2_test) [targets](../using-pants/key-concepts/targets-and-build-files.mdx) to know which Shell files you want to operate on and to set any metadata.
