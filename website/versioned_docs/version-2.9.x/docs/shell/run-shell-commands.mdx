...
The [`experimental_shell_command`](../../reference/targets/experimental-shell-command.mdx) target allows you to run any command during a Pants execution, for the purpose of modifying or creating files to be used by other targets, or its (idempotent: see below) side-effects when accessing services over the network.
...
Unlike `experimental_shell_command`, the [`experimental_run_shell_command` target](../../reference/targets/experimental-run-shell-command.mdx) runs directly in your workspace, without sandboxing.
