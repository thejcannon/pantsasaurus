...
Thirdparty dependencies (i.e. those from repositories like [Maven central](https://search.maven.org/)) are also automatically inferred via dependency inference, but must first be declared once per repository as [`jvm_artifact` targets](doc:reference-jvm_artifact):
...
Pants requires use of a lockfile for thirdparty dependencies. After adding or editing `jvm_artifact` targets, you will need to update affected lockfiles by running `./pants coursier-resolve`. The default lockfile is located at `3rdparty/jvm/default.lock`, but it can be relocated (as well as additional resolves declared) via the [`[jvm].resolves` option](doc:reference-jvm#section-resolves).
...
<TabItem value="src/jvm/org/pantsbuild/example/lib/BUILD" label="src/jvm/org/pantsbuild/example/lib/BUILD">
...
<TabItem value="src/jvm/org/pantsbuild/example/lib/Loader.java" label="src/jvm/org/pantsbuild/example/lib/Loader.java">
...
<TabItem value="src/jvm/org/pantsbuild/example/lib/hello.txt" label="src/jvm/org/pantsbuild/example/lib/hello.txt">
