...
To run an executable/script, use `./pants run` on a [`pex_binary`](reference:reference-pex_binary) target. (See [package](./package.mdx) for more on the `pex_binary` target.)
