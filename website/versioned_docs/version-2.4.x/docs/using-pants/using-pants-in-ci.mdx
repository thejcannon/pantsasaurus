...
{
  "type": "info",
  "title": "Example `.travis.yml`",
  "body": "See https://github.com/pantsbuild/example-python/blob/main/.travis.yml for an example of how to configure Pants with Travis CI."
}
[/block]

[block:api-header]
{
  "title": "`pants.ci.toml` (optional)"
}
[/block]
If you have config that you want to only use in CI, it can be helpful to create a dedicated `pants.ci.toml` [config file](doc:options). For example:
[block:code]
{
  "codes": [
    {
...
[/block]
Then, in your CI script or config, set the environment variable `PANTS_CONFIG_FILES` to use this new config file. Because `--pants-config-files` is a [list option](doc:options), the below will append to the list of config files used, which will include `pants.toml` by default:
[block:code]
{
  "codes": [
    {
      "code": "export PANTS_CONFIG_FILES=pants.ci.toml\n\n# Then, your normal CI setup\n./pants test ::\n...",
      "language": "shell",
      "name": "ci.sh"
    }
  ]
...
[block:api-header]
{
  "title": "Storing Pants logs as artifacts"
}
[/block]
...
[block:code]
{
  "codes": [
    {
      "code": "$ ./pants --version  # This will bootstrap Pants\n$ ./pants --changed-since=origin/main lint\n$ ./pants \\\n  --changed-since=origin/main \\\n  --changed-dependees=transitive \\\n  typecheck test",
      "language": "shell",
      "name": "ci.sh"
    }
  ]
