...
>
  ...
  https://www.pantsbuild.org/v2.4/docs/advanced-target-selection.
</Option>
...
<Option
  ...
  default_repr={`16000000000`}
>
  The maximum size in bytes of the local store containing directories. Stored
  below `--local-store-dir`.
</Option>
...
<Option
  ...
  default_repr={`True`}
>
  If false, Pants will not clean up local directories used as chroots for
  running processes. Pants will log their location so that you can inspect the
  chroot, and run the `__run.sh` script to recreate the process using the same
  argv and environment variables used by Pants. This option is useful for
  debugging.
</Option>
...
>
  ...
  files. See https://www.pantsbuild.org/v2.4/docs/macros.
</Option>
