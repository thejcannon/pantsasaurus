...
If true, don&#x27;t use Shellcheck when running
...
Arguments to pass directly to Shellcheck, e.g. `--shellcheck-args='-e
  SC20529'`.
...
If true, Pants will include all relevant `.shellcheckrc` and `shellcheckrc`
files during runs. See https://www.mankier.com/1/shellcheck#RC_Files for where
