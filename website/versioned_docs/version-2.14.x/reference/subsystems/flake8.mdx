...
<Option
  ...
  default_repr={`[]`}
>
  Arguments to pass directly to Flake8, e.g. `--flake8-args='--ignore E123,W456
  --enable-extensions H111'`.
</Option>
...
<Option
  ...
  default_repr={`[]`}
>
  Paths to extra files to include in the sandbox. This can be useful for Flake8
  plugins, ``` like including config files for the `flake8-bandit` plugin. ```
</Option>
