...
<Field type_repr={`field.type_hint`} default_repr={`'Chart.yaml'`}>
...
<Field type_repr={`field.type_hint`} default_repr={`False`}>
  If set to true, do not push this Helm chart to registries when running
  `/home/josh/work/scie-pants/dist/scie-pants-linux-x86_64 publish`.
</Field>

## `skip_lint`

<Field type_repr={`field.type_hint`} default_repr={`False`}>
  If set to true, do not run any linting in this Helm chart when running
