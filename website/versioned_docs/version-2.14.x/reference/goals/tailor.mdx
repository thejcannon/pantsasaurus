...
<Option
  ...
  default_repr={`False`}
>
  Do not write changes to disk, only write back what would change. Return code 0
  means there would be no changes, and 1 means that there would be.
</Option>
...
<Option
  ...
  default_repr={`None`}
>
  A header, e.g., a copyright notice, to add to the content of created BUILD
  files.
</Option>
...
>
  ...
  type can be a custom target type or a macro that offers compatible
  functionality to the one it replaces (see
  https://www.pantsbuild.org/v2.14/docs/macros).
</Option>
