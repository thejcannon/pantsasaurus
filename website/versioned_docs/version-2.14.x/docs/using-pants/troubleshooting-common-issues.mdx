...
- The dependency exposes a module different than the default Pants uses, e.g. Python's `ansicolors` exposing `colors`.
  ...
  - [JVM](doc:reference-jvm_artifact): set the `packages` field on `jvm_artifact` targets.
...
- Ambiguity. >1 target exposes the same module/package.
  - If it's a third-party dependency, you should likely use multiple "resolves" (lockfiles). Each resolve should have no more than one of the same requirement. See [Python](doc:python-third-party-dependencies#lockfiles#multiple-lockfiles) and [JVM](doc:jvm-overview).
...
- Multiple resolves (Python and JVM).
  ...
  - See [Python](doc:python-third-party-dependencies#lockfiles#multiple-lockfiles) and [JVM](doc:jvm-overview).
