...
To override the default on a particular target, you can use the [`jdk=` field](../../reference/targets/kotlin-source.mdx#codejdkcode).
...
the [`resolve` field](../../reference/targets/kotlin-junit-test.mdx#coderesolvecode) of any targets which should be used with the
...
[`jvm_artifact` targets](../../reference/targets/jvm-artifact.mdx):
...
:::note Thirdparty symbols and the `packages` argument
    ...
    [`jvm_artifact` docs](../../reference/targets/jvm-artifact.mdx#codepackagescode) for more information.
