...
"body": "We'd be happy to ping you on Rust changes we make for you to see how Rust is used in the wild. Please message us on the #engine channel in [Slack](/community/members) to let us know your interest."
}
[/block]
...
"body": "Because changes to Rust deeply impact how Pants runs, it is especially helpful to share any plans to work on Rust before making changes. Please message us on [Slack](/community/members) in the #engine channel or open a [GitHub issue](https://github.com/pantsbuild/pants/issues)."
}
[/block]
...
{
"type": "warning",
"title": "Set `MODE=debug` when iterating on the boundary between Rust and Python",
"body": "As described in [Setting up Pants](./setting-up-pants.mdx), we default to compiling Rust in release mode, rather than debug mode.\n\nWhen working on the boundary between Rust and Python, you typically should set the environment variable `MODE=debug` for substantially faster compiles."
}
[/block]
