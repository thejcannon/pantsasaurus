...
[block:callout]
{
  "type": "info",
  "title": "Why upgrade?",
  "body": "See [Pants 1.x vs. 2.0](doc:pants-1-vs-2)."
}
[/block]

[block:api-header]
{
  "title": "Step 1: Upgrade to Pants 1.30"
}
[/block]
We recommend upgrading to 1.30.1, which is the last minor release in the 1.x series. See [Upgrade tips](doc:upgrade-tips) for some tips with upgrading.
...
[block:api-header]
{
  "title": "Step 2: Setup JVM, Node, and Go support (if relevant)"
}
[/block]
...
[block:api-header]
{
  "title": "Step 3: Rewrite your plugins (if relevant)"
}
[/block]
Refer to [Plugins overview](doc:plugins-overview) to learn more about the new Rules and Target APIs.

We know that rewriting a plugin can be time-consuming, and we want to help. Please message us on [Slack](doc:community) in the `#plugins` channel. We can actively help you rewrite your code, including pair programming if needed. We don't want having to port your plugins to block you from upgrading to 2.0.
[block:api-header]
{
  "title": "Step 4: Upgrade to 2.0"
}
[/block]
...
{
  "type": "info",
  "title": "Reminder: use `ignore_pants_warnings` to ignore deprecations",
...
[block:api-header]
{
  "title": "Step 5: Tweak your CI's caching (recommended)"
}
[/block]
...
Refer to [Using Pants in CI](doc:using-pants-in-ci) for more information, including a script to nuke your cache when it has gotten too large.
[block:api-header]
{
  "title": "Step 6: Set up a constraints file (strongly recommended)"
}
[/block]
...
See [Third-party dependencies](doc:python-third-party-dependencies) for a guide to set this up.
[block:api-header]
{
  "title": "Key differences to be aware of"
}
[/block]
...
{
  "type": "success",
  "title": "Missing features? Let us know!",
  "body": "Please message us on [Slack](doc:community) if you are having issues upgrading or find something missing. We would love to help."
}
[/block]
...
Once you have successfully upgraded to 2.0, you can re-enable dependency inference. You'll first want to teach Pants about your third-party dependencies, see [Third-party dependencies](doc:python-third-party-dependencies). Then set `imports = true`, but don't yet delete anything from your BUILD files.
...
Previously, the `provides` field was used on `python_library` targets for the [`setup-py` goal](doc:python-setup-py-goal). Now, use a dedicated `python_distribution` target.
...
To instead run a test interactively, run `./pants test --debug`. (See [test](doc:python-test-goal)).
...
We also added new options to specify which type of report(s) you'd like. See [test](doc:python-test-goal#coverage) for instructions on how to use coverage.

### Linter config files must be specified (v1 vs. v2 engine)

Because linters and formatters now run in a chroot (temporary directory), you must explicitly specify your config files to make sure that they get copied into the directory. See [Linters and formatters](doc:python-linters-and-formatters).
...
See [typecheck](doc:python-typecheck-goal) for more information.
...
Rather than using a `python_app` target and the `bundle` goal, use the `archive` target type and the `package` goal. We redesigned this feature to be simpler, such as using dependencies on `files()` targets rather than using the `bundle` type. If you still need to relocate where files are located, you can use the `relocated_files()` target. See [Resources and archives](doc:resources).
...
If you're trying to rerun a test, you can instead run `./pants test --force` to force the test to rerun, but still use the cache for everything else  (See [test](doc:python-test-goal)).
...
If you found this feature useful, we'd be happy to add it back in a more powerful way. Please message us on [Slack](doc:community) or open a [GitHub issue](https://github.com/pantsbuild/pants/issues).

### `prep_command` is removed (1.30 vs. 2.0)

`prep_command` does not fit well with the v2 engine's execution model. If you needed this functionality, please message us on [Slack](doc:community) and we will help to figure out how to recreate your setup.

### `minimize`, `filemap`, `path`, `paths`, and `sort` are removed (1.30 vs. 2.0)

We are happy to add these back if you found them useful. Please message us on [Slack](doc:community) or open a [GitHub issue](https://github.com/pantsbuild/pants/issues).