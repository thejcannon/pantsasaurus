...
Most Pants functionality is provided via pluggable [_backends_](doc:enabling-backends). You enable a backend by listing it under the `backend_packages` config key in `pants.toml`.

For example, to [enable Python support](doc:python-backend):
```toml title="pants.toml"
[GLOBAL]
...
...
See [Create initial BUILD files](doc:create-initial-build-files) for more information about how to do that.