...
We recommend running `./pants tailor --check` in your [continuous integration](doc:using-pants-in-ci) so that you don't forget to add any targets and BUILD files.
...
[block:api-header]
{
"title": "Formatting BUILD files with Black or Yapf"
}
[/block]
...
Like `tailor`, we recommend running `./pants update-build-files --check` in your [continuous integration](doc:using-pants-in-ci).
