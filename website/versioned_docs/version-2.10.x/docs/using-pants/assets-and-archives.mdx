...
A [`resource`](reference:reference-resource) target is for files that are members of code packages, and are loaded via language-specific mechanisms, such as Python's `pkgutil.get_data()` or Java's `getResource()`.
...
To reduce boilerplate, the [`resources`](reference:reference-resources) target generates a `resource` target per file in the `sources` field.
...
<TabItem value="src/python/project/config.json" label="src/python/project/config.json">
...
To reduce boilerplate, the [`files`](reference:reference-files) target generates a `file` target per file in the `sources` field.

For example, to load loose files in Python:

<Tabs>
<TabItem value="src/python/project/app_test.py" label="src/python/project/app_test.py">
...
The `files` field is a list of `file`, `files`, and `relocated_files` targets. See [resources](#resources) for more details.
