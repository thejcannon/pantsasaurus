...
[block:api-header]
{
  "title": "Debug tip: inspect the sandbox with `--no-process-cleanup`"
}
[/block]
...
* Pants does not know about some of your project files because you do not have targets and BUILD files describing them.
     * Tip: run [`./pants tailor`](doc:create-initial-build-files), which will generate targets for files without owners. Note that this will not add every target type, such as [`resources` and `files`](doc:assets).
...
* Some targets can never be inferred and must always be explicitly added via the `dependencies` field, e.g. [`file` / `files` and `resource` / `resources` targets](doc:assets).
* For Python third-party requirements, check if they have any [undeclared dependencies](doc:python-third-party-dependencies#advanced-usage).
...
If `./pants dependencies` is behaving how you'd expect, but certain imports of first-party code are still missing, try running with `--no-process-cleanup` and inspect the logged sandbox to see if the files you're expecting are present. If files are missing, you may need to adjust the [`sources` field](doc:targets#sources) of some of your dependencies.
[block:api-header]
{
...
[block:api-header]
{
  "title": "\"Double requirement given\" error when resolving Python requirements"
}
[/block]
...
[block:code]
{
  "codes": [
    {
      "code": "# Check the `requirements` key to see if it has the problematic requirement.\n./pants filter --target-type=python_requirement | xargs ./pants peek",
      "language": "shell",
      "name": "Shell"
    }
  ]
...
[block:api-header]
{
  "title": "Controlling (test) parallelism"
}
[/block]
