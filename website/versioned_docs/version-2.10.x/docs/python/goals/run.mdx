...
To run an executable/script, use `./pants run` on a [`pex_binary`](../../../reference/targets/pex-binary.mdx) target. (See [package](./package.mdx) for more on the `pex_binary` target.)
