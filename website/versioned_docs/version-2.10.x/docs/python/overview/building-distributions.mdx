...
You configure a distribution using a [`python_distribution`](reference:reference-python_distribution) target. This target provides Pants with the information needed to build the distribution.
...
:::note Use the `entry_points` field to register entry points like `console_scripts`
    ...
    The [`entry_points` field](reference:reference-python-distributions#codeentry_pointscode) allows you to configure [setuptools-style entry points](https://packaging.python.org/specifications/entry-points/#entry-points-specification):
...
:::note How to publish your distributions to a package index
    ...
    See [publish](reference:reference-publish) for example support publishing distributions using Twine.
