...
<Field type_repr={`field.type_hint`}>
  A timeout (in seconds) used by each test file belonging to this target. If
  unset, the test will never time out.
</Field>

## `shell`

<Field type_repr={`field.type_hint`}>
  Which shell to run the tests with. If unspecified, Pants will look for a
  shebang line.
</Field>
