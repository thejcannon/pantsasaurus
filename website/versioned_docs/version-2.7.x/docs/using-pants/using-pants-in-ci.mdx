...
- [`process_execution_local_parallelism`](reference:reference-global#section-process-execution-local-parallelism): number of concurrent processes that may be executed locally.
- [`rule_threads_core`](reference:reference-global#section-rule-threads-core): number of threads to keep active to execute `@rule` logic.
- [`rule_threads_max`](reference:reference-global#section-rule-threads-max): maximum number of threads to use to execute `@rule` logic.
- [`[python-setup].resolver_num_jobs`](reference:reference-python-setup#section-resolver-jobs): maximum number of concurrent jobs to build Python wheels with.

Memory usage options:

- [`pantsd`](reference:reference-global#section-pantsd): enable or disable the Pants daemon, which uses an in-memory cache to speed up subsequent runs after the first run in CI.
- [`pantsd_max_memory_usage`](reference:reference-global#section-pantsd-max-memory-usage): reduce or increase the size of Pantsd's in-memory cache.
