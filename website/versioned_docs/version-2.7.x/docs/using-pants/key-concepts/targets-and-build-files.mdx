...
[/block]
Each target type has different \_fields*, or individual metadata values. Run `./pants help $target` to see which fields a particular target type has, e.g. `./pants help files`. Most fields are optional and use sensible defaults.
...
"body": "See [Adopting Pants in an Existing Repo](../../learn-about-pants/getting-started/incremental-adoption.mdx) for tips on setting up Pants, including where to add BUILD files."
}
[/block]
