...
A distribution is set up using a [`python_distribution`](reference:reference-python_distribution) target. This target provides Pants with the information needed to build the distribution, such as the setuptools commands to run.
