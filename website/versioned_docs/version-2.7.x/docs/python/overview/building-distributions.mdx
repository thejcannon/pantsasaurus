...
:::note See `package` for other package formats
    ...
    This page focuses on building sdists and wheels with the `./pants package` goal. See [package](doc:python-package-goal) for information on other formats that work with `./pants package`, such as PEX binaries and zip/tar archives.
...
:::note Use the `entry_points` field to register entry points like `console_scripts`
    ...
    The [`entry_points` field](doc:reference-python-distributions#codeentry_pointscode) allows you to configure [setuptools-style entry points](https://packaging.python.org/specifications/entry-points/#entry-points-specification):
