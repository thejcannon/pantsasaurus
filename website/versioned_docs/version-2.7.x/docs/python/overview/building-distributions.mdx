...
A distribution is set up using a [`python_distribution`](doc:reference-python_distribution) target. This target provides Pants with the information needed to build the distribution, such as the setuptools commands to run.
...
[block:callout]
{
  ...
  "body": "This page focuses on building sdists and wheels with the `./pants package` goal. See [package](doc:python-package-goal) for information on other formats that work with `./pants package`, such as PEX binaries and zip/tar archives."
}
[/block]

[block:api-header]
{
  "title": "Using an existing `setup.py`"
}
[/block]
...
[block:api-header]
{
  "title": "Mapping libraries to distributions"
}
[/block]
...
[block:api-header]
{
  "title": "Viewing the generated `setup.py` file"
}
[/block]
...
{
  "type": "info",
  "title": "How to upload your distributions to a package index",
  "body": "Pants does not have a mechanism to upload your distributions. Instead, run `package` as described above to create the asset, such as a `.whl` file, then use a tool like Twine to upload your package.\n\nPlease let us know on [Slack](doc:community) or [GitHub issues](https://github.com/pantsbuild/pants/issues/new) if you would like this feature."
}
[/block]