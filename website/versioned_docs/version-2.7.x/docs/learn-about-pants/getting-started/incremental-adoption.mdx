...
{
"type": "info",
"body": "We would love to help you with adopting Pants. Please reach out through [Slack](doc:community).",
"title": "Joining Slack"
}
[/block]
...
Follow the [Getting Started](doc:getting-started) guide to install Pants and [set up an initial `pants.toml`](doc:initial-configuration). Validate that running `./pants count-loc '**'` works properly.
...
First, create a BUILD file in each directory with Python code. As explained in [Targets and BUILD files](doc:targets), you can use fewer BUILD files, such as one BUILD file per subproject; but we find that one BUILD file per directory is a good default.
[block:callout]
{
"type": "success",
"title": "Use `./pants tailor` to cut down on manual work",
"body": "The `tailor` goal will generate initial BUILD files for you! You may need to tweak them manually, but for the most part the auto-generated ones should get you off to a good start. You can run `./pants tailor` again as you add new files in the future.\n\nSee [Creating Initial BUILD files](doc:create-initial-build-files) for more information."
}
[/block]
...
Then, activate the Python [Linters and formatters](doc:python-linters-and-formatters) you'd like to use. Hook up the `fmt` and `lint` goals to your [CI](doc:using-pants-in-ci).

### 3. Set up tests

To get [tests](doc:python-test-goal) working, you will first need to [set up third-party dependencies](doc:python-third-party-dependencies). We strongly recommend setting up a lockfile for reproducibility and better performance.
...
However, if your tests depend on any resource files, you will need to explicitly declare these dependencies because Pants cannot infer them. See [Resources](doc:resources).
...
In your CI, you may want to speed up your build by having Pants only run tests for changed files. Refer to [Using Pants in CI](doc:using-pants-in-ci).

### 4. Set up `./pants package`

You can use `./pants package` to package your code into various formats, such as a [PEX binary](doc:python-package-goal), a [wheel](doc:python-distributions), an [AWS Lambda](doc:awslambda-python), or a [zip/tar archive](doc:resources).
