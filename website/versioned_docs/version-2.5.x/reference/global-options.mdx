...
(https://www.pantsbuild.org/v2.5/docs/advanced-target-selection).
...
files. See (https://www.pantsbuild.org/v2.5/docs/macros).
...
<Option
  ...
  removal_hint={
    "Use the global option `--ignore-warnings` instead.<br /><br />Unlike this option, `--ignore-warnings` uses literal string matches instead of regex patterns by default. If you would still like to use a regex pattern, prefix the string with `$regex$`."
  }
>
