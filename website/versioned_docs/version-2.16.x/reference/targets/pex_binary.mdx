...
<Field type_repr={`field.type_hint`} default_repr={`True`}>
  Whether to include your first party sources the binary uses in the packaged
  PEX file.
</Field>
...
<Field type_repr={`field.type_hint`}>
  Freeze these command-line args into the PEX. Allows you to run generic entry
  points on specific arguments without creating a shim file.
</Field>

## `env`

<Field type_repr={`field.type_hint`}>
  Freeze these environment variables into the PEX. Allows you to run generic
  entry points on a specific environment without creating a shim file.
</Field>
