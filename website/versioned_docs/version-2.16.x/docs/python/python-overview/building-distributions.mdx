...
:::note Use the `entry_points` field to register entry points like `console_scripts`
    ...
    The [`entry_points` field](reference:reference-python_distribution#codeentry_pointscode) allows you to configure [setuptools-style entry points](https://packaging.python.org/specifications/entry-points/#entry-points-specification):
