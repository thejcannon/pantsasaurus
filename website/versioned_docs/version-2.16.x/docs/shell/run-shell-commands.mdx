...
The [`shell_command`](../../reference/targets/shell-command.mdx) target allows you to run any command during a Pants execution, for the purpose of modifying or creating files to be used by other targets, or its (idempotent: see below) side-effects when accessing services over the network.
...
Unlike `shell_command`, the [`run_shell_command` target](../../reference/targets/run-shell-command.mdx) runs directly in your workspace, without sandboxing.
