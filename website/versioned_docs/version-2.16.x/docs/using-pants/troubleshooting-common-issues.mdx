...
:::tip We love giving help!
    ...
    See [Getting Help](../getting-help/getting-help.mdx). We would love to help!
...
Once you have this stack trace, we recommend copying it into Pastebin or a GitHub Gist, then opening a GitHub issue or posting on Slack. Someone from the Pants team would be happy to help. See [Getting Help](../getting-help/getting-help.mdx).
...
- The dependency exposes a module different than the default Pants uses, e.g. Python's `ansicolors` exposing `colors`.
  ...
  - [JVM](reference:reference-jvm-artifact): set the `packages` field on `jvm_artifact` targets.
...
A more sustainable solution for shared resources is to use the [`[pytest].execution_slot_var`](reference:reference-pytest#section-execution-slot-var) option, which sets an environment variable which test runs can consume to determine which copy of a resource to consume.
