...
<Field type_repr={`field.type_hint`}>
  A list of addresses to `resources` and `files` targets with content to place
  in the document root of this WAR file.
</Field>
...
<Field type_repr={`field.type_hint`}>
  Path to a file containing the descriptor (i.e., web.xml) for this WAR file.
  Defaults to `web.xml`.
</Field>
