...
<Option
  ...
  default_repr={`False`}
>
  Whether to log the Docker output to the console. If false, only the image ID
  is logged.
</Option>
...
>
  ...
  these tools will be included in the PATH used in the execution sandbox, so
  that they may be used by the Docker client.
</Option>
