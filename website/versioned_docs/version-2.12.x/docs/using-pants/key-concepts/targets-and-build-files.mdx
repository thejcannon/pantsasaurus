...
Use [`./pants tailor`](../../getting-started/getting-started/initial-configuration.mdx#5-generate-build-files) to automate generating BUILD files, and [`./pants update-build-files`](reference:reference-update-build-files) to reformat them (using `black`, [by default](reference:reference-update-build-files#section-formatter)).
