...
3. Specify which shell to run your tests with, either by setting a shebang directly in the test file or by setting the field `shell` on the `shunit2_test` / `shunit2_tests` target. - See [here](../../reference/targets/shunit2_tests.mdx#codeshellcode) for all supported shells.
   [block:code]
   {
   "codes": [
   {
