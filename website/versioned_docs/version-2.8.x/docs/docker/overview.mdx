...
Pants uses [`docker_image`](reference:reference-docker_image) [targets](../using-pants/key-concepts-to-know-about-pants/targets-and-build-files.mdx) to indicate which Dockerfiles you want Pants to know about, and to add any necessary metadata.
...
A [`docker_image`](reference:reference-docker_image) can depend on loose files belonging to [`file` / `files` targets](../using-pants/resources-and-archives.mdx#files), and on artifacts packaged from a variety of targets, such as [`pex_binary`](reference:reference-pex_binary) , [`python_distribution`](reference:reference-python_distribution), [`archive`](reference:reference-archive), or any other target that can be built via the [package](reference:reference-package) goal.
