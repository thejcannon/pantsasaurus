...
<TabItem value="src/example/BUILD" label="src/example/BUILD">
...
(Note that the field is named `image_tags` and not just `tags`, because Pants has [its own tags concept](reference:reference-target#codetagscode), which is unrelated.)
...
<TabItem value="src/example/Dockerfile" label="src/example/Dockerfile">
...
:::note Dynamically generated tags
    ...
    Generating custom tags at runtime based on Git state or image inputs currently requires you to write some custom plugin code. Don't hesitate to [reach out](../getting-help/getting-help.mdx) for help with this.
