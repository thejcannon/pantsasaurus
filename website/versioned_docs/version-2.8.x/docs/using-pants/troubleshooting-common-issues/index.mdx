...
[block:api-header]
{
  "title": "Debug tip: inspect the sandbox with `--no-process-execution-local-cleanup`"
}
[/block]
...
* Some targets can never be inferred and must always be explicitly added via the `dependencies` field, e.g. [`file` / `files` and `resource` / `resources` targets](doc:resources).
...
If `./pants dependencies` is behaving how you'd expect, but certain imports of first-party code are still missing, try running with `--no-process-execution-local-cleanup` and inspect the logged sandbox to see if the files you're expecting are present. If files are missing, you may need to adjust the [`sources` field](doc:targets#sources) of some of your dependencies.
[block:api-header]
{
