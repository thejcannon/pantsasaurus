...
```
 * Tip: run [`./pants tailor`](reference:reference-tailor), which will generate targets for files without owners. Note that this will only teach Pants about your first-party code, but not third-party requirements, which need to be manually added.
```
