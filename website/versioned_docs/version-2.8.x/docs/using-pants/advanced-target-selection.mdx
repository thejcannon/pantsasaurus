...
:::caution Using a version control system other than Git?
    ...
    Please message us on Slack or open a GitHub issue (see [Community](../getting-help/getting-help.mdx)). We would be happy to look into adding support for your VCS, such as helping you with a PR to add support.
...
You can then filter by tags with the global `--tag` [option](reference:reference-global#section-tag), like this:
...
<TabItem value="targets.txt" label="targets.txt">
