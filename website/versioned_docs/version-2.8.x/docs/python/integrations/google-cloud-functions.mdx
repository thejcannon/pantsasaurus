...
{
  "type": "warning",
  "body": "`file` / `files` targets will not be included in the built Cloud Function because filesystem APIs like `open()` would not load them as expected. Instead, use the `resource` / `resources` target. See [Resources and archives](doc:resources) for further explanation.",
