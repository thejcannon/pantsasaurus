...
{
  "type": "warning",
  "body": "`file` and `files` targets will not be included in the built PEX because filesystem APIs like `open()` would not load them as expected. Instead, use the `resource` and `resources` target or wrap your `pex_binary` in an `archive` target. See [Resources and archives](doc:resources) for further explanation.",
...
[/block]
See [Resources and archives](doc:resources) for how to create a zip or tar file with built binaries and/or loose files in it by using the `archive` target. 
