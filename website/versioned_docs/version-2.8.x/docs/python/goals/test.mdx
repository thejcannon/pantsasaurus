...
"body": "Add [`pytest-icdiff`](https://github.com/hjwp/pytest-icdiff#usage) and [`pygments`](https://github.com/pygments/pygments) to the `[pytest].extra_requirements` option for better error messages and diffs from Pytest."
}
[/block]
...
Refer to [Resources](doc:resources) for how to include resource files in your tests by adding to the `dependencies` field.
