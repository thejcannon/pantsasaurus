...
[/block]
Pants's [dependency inference](doc:targets) understands imports of the `pants` module and will automatically add dependencies on the `pants_requirement` where relevant.
[block:callout]
{
  "type": "warning",
  "title": "`pants_requirement` cannot update a constraints file",
  "body": "If you are using a lockfile—which we recommend for both correctness and performance—then you will need to remember to regenerate your `constraints.txt` file when upgrading Pants versions.\n\nSee [Third-party dependencies](doc:python-third-party-dependencies)."
}
[/block]
