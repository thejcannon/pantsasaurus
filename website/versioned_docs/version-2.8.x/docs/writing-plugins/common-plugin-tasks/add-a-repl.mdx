...
[block:callout]
{
  ...
  "body": "This guide walks through adding a simple `repl` implementation for Bash that runs `/bin/bash` and ensures all relevant files are included in the temporary directory. See [here](https://github.com/pantsbuild/example-plugin/blob/main/pants-plugins/examples/bash/repl.py) for the final implementation."
}
[/block]
