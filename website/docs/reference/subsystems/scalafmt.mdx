...
Backend: <span style={{ color: "var(--ifm-color-primary)" }}>`pants.backend.experimental.scala.lint.scalafmt`</span>

Config section: <span style={{ color: "var(--ifm-color-primary)" }}>`[scalafmt]`</span>
...
Set to the string `\<default>` to use a lockfile provided by Pants, so long as you have not changed the `--version` option. See https\://github.com/pantsbuild/pants/blob/release_2.18.0rc2/src/python/pants/backend/scala/lint/scalafmt/scalafmt.default.lockfile.txt for the default lockfile contents.
