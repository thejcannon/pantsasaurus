...
:::caution Kotlin support is alpha stage
    ...
    Please share feedback for what you need to use Pants with your Kotlin project by either [opening a GitHub issue](https://github.com/pantsbuild/pants/issues/new/choose) or [joining our Slack](../getting-help/the-pants-community/index.mdx)!
...
To use multiple Kotlin versions in a repository, you would define multiple resolves, and then adjust the [`resolve` field](reference:reference-kotlin_junit_test#coderesolvecode) of any targets which should be used with the non-`default_resolve` resolve.
...
Third-party dependencies (i.e. those from repositories like [Maven central](https://search.maven.org/)) are also automatically inferred via dependency inference, but must first be declared once per repository as [`jvm_artifact` targets](reference:reference-jvm_artifact):
...
:::note Thirdparty symbols and the `packages` argument
    ...
    The `packages` argument allows you to override which symbols a `jvm_artifact` provides. See the [`jvm_artifact` docs](reference:reference-jvm_artifact#codepackagescode) for more information.
