...
Pants uses [`docker_image`](reference:reference-docker_image) [targets](../using-pants/key-concepts/targets-and-build-files.mdx) to indicate which Dockerfiles you want Pants to know about, and to add any necessary metadata.
...
Alternatively you may provide the Docker build instructions inline in your BUILD file as [`instructions`](reference:reference-docker_image#codeinstructionscode) on your `docker_image` if you don't want to create a `Dockerfile`.
...
Pants, however, takes care of assembling the context for you. It does so using the dependencies of the [`docker_image`](reference:reference-docker_image) target, which can include:

- Loose files specified using [`file` / `files` targets](../using-pants/assets-and-archives.mdx#files).
- Artifacts packaged from a variety of targets, such as [`pex_binary`](reference:reference-pex_binary) , [`python_distribution`](reference:reference-python_distribution), [`archive`](reference:reference-archive), and any other target that can be built via the [package](reference:reference-package) goal, including other docker images.
...
When your `Dockerfile` is a multi-stage build file, you may specify which stage to build with the [`--docker-build-target-stage`](reference:reference-docker#section-build-target-stage) for all images, or provide a per image setting with the `docker_image` field [`target_stage`](reference:reference-docker_image#codetarget_stagecode).
...
Secrets are supported for `docker_image` targets with the [`secrets`](reference:reference-docker_image#codesecretscode) field. The defined secrets may then be mounted in the `Dockerfile` as [usual](https://docs.docker.com/develop/develop-images/build_enhancements/#new-docker-build-secret-information).
...
:::note Secret file path
    ...
    See the example for the [`secrets`](reference:reference-docker_image#codesecretscode) field.
